...
// END PA 4 FUNCTIONS
  ...
  private:
	...
	// private helper method for finding a node at a given index or finding its
    // deepest ancestor
    ...
    //                                 traversal
	QuadtreeNode* findNode(QuadtreeNode* node, int x, int y, int middle) const;
    ...
    //   - return:     nothing
    void clockwiseShift(QuadtreeNode* &node);
    ...
    //                                pruning is done
    //   - returns: nothing
    void recPrune(QuadtreeNode* &subroot, int tolerance);
    ...
    int recPruneSize(QuadtreeNode* subroot, int tolerance) const;

	// Helper function for recPrune and recPruneSize that gets all the leaves of a particular subroot
    ...
    //   - returns: nothing
    void getLeaves(QuadtreeNode* subroot, std::vector<QuadtreeNode*> &leaves) const;

	// Helper method for recPrune and recPruneSize that checks all of the leaves of a subroot against the tolerance
    ...
    bool checkLeaves(QuadtreeNode* subroot, std::vector<QuadtreeNode*> leaves, int tolerance) const;

	// Helper function for recPrune and recPruneSize which calculates the difference between two RGB colors
    //   - parameters: QuadtreeNode* subrot - pointer to a subroot in a Quadtree object
    //   - parameters: QuadtreeNode* child - pointer to one of the four children of the subroot
    //   - returns: the integer value of the differnce between the two colors
    int calcDifference(QuadtreeNode* subroot, QuadtreeNode* child) const;

    // Helper function for idealPrune using the Binary Search
    //   - parameters: int min  - minimum tolerance
    //   - parameters: int max  - maximum tolerance
    //   - parameters: int numLeaves - the number of leaves you want to remain in the tree
    int idealPrune(int min, int max, int numLeaves) const;

    // copy helper function for Quadtree copy constructor
    QuadtreeNode* copy(QuadtreeNode* cRoot);

    // clear helper function for Quadtree destructor
    void clear(QuadtreeNode *& subroot);

    // btHelper function for buildTree
    void btHelper(PNG const & source, int xcoord, int ycoord, int resolution, QuadtreeNode *&subroot);
